...
public class IssueQueryActivity extends BaseTransactionQueryActivity{

	private ListGrid mGrid;

	private IssueDao mDao;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		columns.add(new GridColumn("现有数量", (int) (120 * density)));
		columns.add(new GridColumn("发料数量", (int) (120 * density)));
		...
		mDao = IssueDao.getInstance(this);
	}
		...
		try {
			...
			if(list != null && list.size()>0){
				...
				for(Issue item : list){
					...
					row.setValue(4, DateFormat.defaultFormat(item.complete_time));
					...
					row.setValue(10, item.remaining_quantity + "");
					row.setValue(11, item.transaction_quantity + "");
					row.setValue(12, DateFormat.defaultFormat(item.create_time));
					row.setValue(13, item.create_by);
			...
			ToastHelper.getInstance(IssueQueryActivity.this).show(e.getMessage());
		}
	}
}
